...
void Blob::Reshape(const vector<int>& shape, bool reinitialize) {
...
//mkldnn
const real_t* Blob::prv_data() const {
  CHECK(data_);
  return (const real_t*)data_->prv_data();
}

real_t* Blob::mutable_prv_data() {
  CHECK(data_);
  return static_cast<real_t*>(data_->mutable_prv_data());
}

void Blob::set_prv_data_descriptor(shared_ptr<PrvMemDescr> descriptor,
         bool same_data) {
    CHECK(data_);
...
shared_ptr<PrvMemDescr> Blob::get_prv_data_descriptor() {
  CHECK(data_);
