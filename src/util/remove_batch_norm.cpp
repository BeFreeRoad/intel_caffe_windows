...
void RecoverScaleFromBN(const LayerParameter& bn_layer_param, LayerParameter& scale_layer_param, real_t default_scale_weights, real_t default_scale_bias) {
  ...
  if (bn_layer_blob_size == 5) {
    ...
    scale_layer_param.add_blobs()->CopyFrom(bn_layer_param.blobs(3));
    Blob scale_bias_blob, scale_weight_blob;
    ...
    scale_bias_blob.ToProto(&scale_bias_blob_proto);
    scale_layer_param.add_blobs()->CopyFrom(scale_bias_blob_proto);
  } else {
    Blob scale_weight_blob, scale_bias_blob, bn_mean_blob;
    ...
    caffe_set(scale_bias_blob.count(), default_scale_bias, scale_bias_blob.mutable_cpu_data());
    scale_weight_blob.ToProto(&scale_weight_blob_proto);
    scale_bias_blob.ToProto(&scale_bias_blob_proto);
...
void AdjustConvLayer(LayerParameter &conv_layer,
  ...
  if (is_net_init) {
    ...
    Blob conv_weight_blob, conv_bias_blob;
    Blob scale_weight_blob, scale_bias_blob;
    Blob bn_mean_blob, bn_variance_blob, bn_scale_factor_blob;
    real_t bn_scale_factor;
    real_t bn_eps = batch_norm_layer.batch_norm_param().eps();
    ...
    if (!conv_layer.convolution_param().bias_term()) {
      ...
      caffe_set(conv_bias_blob.count(), (real_t)0, conv_bias_blob.mutable_cpu_data());
      BlobProto conv_bias_blob_proto;
      conv_bias_blob.ToProto(&conv_bias_blob_proto);
    ...
    real_t alpha, scale_weight_val, bn_variance_val;
    real_t * conv_weight_buf = conv_weight_blob.mutable_cpu_data();
    real_t * conv_bias_buf = conv_bias_blob.mutable_cpu_data();
    const real_t * scale_bias_buf = scale_bias_blob.cpu_data();
    const real_t * bn_mean_buf = bn_mean_blob.cpu_data();
    ...
    for (int i = 0; i < alpha_count; i++) {
      ...
      real_t * weight_area = conv_weight_buf + i * weight_count;
...
void RecoverBNScaleMergedNet(NetParameter * net_param, NetParameter* recovered_net_param) {
  ...
  for (int i = 0; i < net_param->layer_size(); ++i) {
    ...
    if (layer_param.type().compare("BatchNorm") == 0 && layer_param.blobs_size() >= 3) {
      ...
      RecoverScaleFromBN(layer_param, *scale_layer_param, (real_t)1, (real_t)0);
...
void RemoveBNScale(const NetParameter& param, NetParameter* param_compiled) {
  ...
  for (int i = 0; i < param.layer_size(); ++i) {
    ...
    if (layer_param->type().compare("Convolution") == 0) {
      ...
      Net::GetBlobConsumers(child_layers_params, layer_param->top(0), param, i + 1 < param.layer_size() ? i + 1 : i);
      ...
      if (child_layer_param.type().compare("BatchNorm") == 0) {
        ...
        Net::GetBlobConsumers(grandchild_layers_params, child_layer_param.top(0), param, i + 2 < param.layer_size() ? i + 2 : i);
        ...
        if (grandchild_layer_param.type().compare("Scale") == 0) {
          ...
          AdjustConvLayer(*layer_param, child_layer_param, grandchild_layer_param, is_net_init);
          ...
          if (!is_net_init) {
            shared_ptr<LayerParameter> scale_layer_param(new LayerParameter());
            RecoverScaleFromBN(child_layer_param, *scale_layer_param, (real_t)1, (real_t)0);
            AdjustConvLayer(*layer_param, child_layer_param, *scale_layer_param, is_net_init);
          } else {
            AdjustConvLayer(*layer_param, child_layer_param, grandchild_layer_param, true);
          }
          if (bn_scale_remove == false) bn_scale_remove = true;
          layers_to_drop.insert(child_layer_param.name());
        }
...
void RecoverScaleFromBN(const LayerParameter& bn_layer_param, LayerParameter& scale_layer_param, float default_scale_weights, float default_scale_bias);
void AdjustConvLayer(LayerParameter &conv_layer,
                     const LayerParameter &batch_norm_layer,
                     const LayerParameter &scale_layer, bool is_net_init);
